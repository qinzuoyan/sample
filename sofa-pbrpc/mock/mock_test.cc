...
#include <sample/sofa-pbrpc/mock/echo_service.pb.h>
...
{
    LOG(INFO) << "MockEchoSuccess() called";
...
TEST_F(MockTest, test_registered_mock_sync_success)
{
    ...
    LOG(INFO) << "ErrorText: " << cntl->ErrorText();
...
{
    LOG(INFO) << "EchoCallback(): %s" << cntl->ErrorText();
    ASSERT_TRUE(cntl->Failed());
    ASSERT_EQ(RPC_ERROR_FROM_USER, cntl->ErrorCode());
    ASSERT_EQ("RPC_ERROR_FROM_USER: mock failed", cntl->ErrorText());
    *callbacked = true;
